...
init([Partition]) ->
    ...
    PreparedTxs = tx_utilities:open_private_table(prepared),
    CommittedTxs = tx_utilities:open_private_table(committed),
    InMemoryStore = tx_utilities:open_private_table(inmemory_store),
    DepDict = dict:new(),
        ...
        false ->
            case local_cert_util:ready_or_block(TxId, Key, PreparedTxs, {TxId, ignore, {relay, Reader}}) of
                not_ready->
                    ...
                    {noreply, SD0};
                ready ->
                      lager:warning("Read for ~w of ~w finished! Replying to ~w", [Key, TxId, Reader]),
                    Result = read_value(Key, TxId, InMemoryStore),
        ...
        true ->
            ...
            case local_cert_util:specula_read(TxId, Key, PreparedTxs, {TxId, ignore, {relay, Reader}}) of
                not_ready->
                    ...
                    {noreply, SD0};
                {specula, Value} ->
                    gen_server:reply(Reader, {specula, Value}), 
    		        %T2 = os:timestamp(),
...
terminate(_Reason, #state{prepared_txs=PreparedTxs, committed_txs=CommittedTxs, inmemory_store=InMemoryStore} = _State) ->
...
commit(TxId, TxCommitTime, CommittedTxs, PreparedTxs, InMemoryStore, DepDict, Partition, IfSpecula)->
    ...
    case ets:lookup(PreparedTxs, TxId) of
        [{TxId, Keys}] ->
            case IfSpecula of
                true -> specula_utilities:deal_commit_deps(PreparedTxs, TxId, TxCommitTime);
                false -> ok
            end,
