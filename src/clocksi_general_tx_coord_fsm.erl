...
execute_batch_ops(SD=#state{causal_clock=CausalClock,
    ...
    ProcessOp = fun(Operation, {UpdatedParts, RSet, Buffer, TmpReadStat, TmpPrepareBegin}) ->
                    case Operation of
                        ...
                        {update, Key, Type, Op} ->
                            Preflist = ?LOG_UTIL:get_preflist_from_key(Key),
                            IndexNode = hd(Preflist),
                            NewPrepareBegin = tx_utilities:now_microsec(),
    ...
    case dict:size(WriteSet1) of
        ...
        1->
            ...
            SD#state{state=committing, num_to_ack=1, read_set=ReadSet1, tx_id=TxId, prepare_stat=[],
                read_stat=ReadStat, prepare_begin=PrepareBegin}};
        N->
            ...
            {next_state, receive_prepared, SD#state{num_to_ack=N, state=prepared,
                 updated_partitions=WriteSet1, read_set=ReadSet1, tx_id=TxId, prepare_stat=[],
                read_stat=ReadStat, prepare_begin=PrepareBegin}}
    end.
    ...
    case NumToAck of 
        1 ->
            send_commit(S0#state{prepare_time=MaxPrepareTime, commit_time=MaxPrepareTime, 
                    state=committing, 
               prepare_stat=[(tx_utilities:now_microsec()-PrepareBegin)|PrepareStat]});
        _ ->
...
reply_to_client(SD=#state{from=From, tx_id=TxId, state=TxState, read_set=ReadSet, final_read_stat=FinalReadStat,
                ...
                final_prepare_stat=FinalPrepareStat, commit_time=CommitTime, operations=Operations}) ->
    case TxState of
        committed ->
            case length(Operations) of 
                ...
                _ ->
                    ...
                    execute_batch_ops(SD#state{operations=RestOps, final_read_stat=[ReadStat|FinalReadStat],
                        final_read_set=[ReadSet|FinalReadSet], final_prepare_stat=[PrepareStat|FinalPrepareStat],
                        causal_clock=CommitTime})
            end;
        aborted ->
