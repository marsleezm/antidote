...
init([Name]) ->
    ...
    {ok, #state{pending_txs=PendingTxs, dep_dict=dict:new(), total_repl_factor=TotalReplFactor, cdf=Cdf, 
            specula_length=SpeculaLength, specula_read=SpeculaRead, rep_dict=RepDict, name=Name}}.
                        ...
                        _ ->
                            ...
                            case length(PendingList) >= SpeculaLength of
                                true -> %% Wait instead of speculate.
                                    ...
                                    {noreply, SD0#state{tx_id=TxId, dep_dict=DepDict1, local_updates=[], lp_start=StartTime, rp_start=StartTime, num_specula_read=NumSpeculaRead1, remote_updates=RemoteUpdates, sender=Sender, stage=remote_cert}};
                                false -> %% Can speculate. After replying, removing TxId
                                    ...
                                    ets:insert(PendingTxs, {TxId, {[], RemotePartitions, StartTime, StartTime, no_wait}}),
        ...
        [PendingTxId|Rest] ->
            ...
            case decide_after_cascade(NewPendingList, DepDict2, NumAborted, CurrentTxId, Stage) of
                ...
                specula -> %% Can not commit, but can specualte (due to the number of specula txns decreased)
                    ...
                    RemoteParts = [P||{P, _} <-RemoteUpdates],
                    ets:insert(PendingTxs, {CurrentTxId, {LocalParts, RemoteParts, LT, RT, waited}}),
