...
handle_call({certify, TxId, LocalUpdates, RemoteUpdates},  Sender, SD0=#state{rep_dict=RepDict, read_aborted=ReadAborted, read_invalid=ReadInvalid, total_repl_factor=TotalReplFactor, committed=Committed, pending_txs=PendingTxs, min_commit_ts=LastCommitTs, tx_id=TxId,  invalid_ts=InvalidTs, specula_length=SpeculaLength, pending_list=PendingList, dep_dict=DepDict, num_specula_read=NumSpeculaRead}) ->
                        ...
                        _ ->
                            ...
                            case length(PendingList) >= SpeculaLength of
                                ...
                                false -> %% Can speculate. After replying, removing TxId
                                    ...
                                    {NewRemoteUps, AvoidNum} = add_to_table(RemoteUpdates, TxId, LastCommitTs, RepDict, [], 0),
...
handle_cast({clean_data, Sender}, #state{pending_txs=OldPendingTxs}) ->
    ...
    TotalReplFactor = length(Replicas)+1,
    Sender ! cleaned,
    {noreply, #state{pending_txs=PendingTxs, dep_dict=dict:new(), total_repl_factor=TotalReplFactor, 
            do_repl=DoRepl, specula_length=SpeculaLength, rep_dict=RepDict}};
        ...
        {ok, {1, ReadDepTxs, OldPrepTime}} ->
            ...
            case length(PendingList) >= SpeculaLength of
                ...
                false ->
                    ...
                    {NewRemoteUps, AvoidNum} = add_to_table(RemoteUpdates, TxId, NewMaxPrep, RepDict, [], 0),
                    ?CLOCKSI_VNODE:prepare(NewRemoteUps, TxId, {remote, node()}),
...
handle_cast({prepared, TxId, PrepareTime}, 
	    SD0=#state{tx_id=TxId, local_updates=LocalParts, do_repl=DoRepl, stage=local_cert, total_repl_factor=TotalReplFactor, 
                ...
                false ->
                    ...
                    case length(PendingList) >= SpeculaLength of
                        ...
                        false ->
                            ...
                            %% Add dependent data into the table
                            ets:insert(PendingTxs, {TxId, {LocalParts, RemoteParts, os:timestamp()}}),
                            {NewRemoteUps, NumAvoid} = add_to_table(RemoteUpdates, TxId, NewMaxPrep, RepDict, [], 0),
                            ?CLOCKSI_VNODE:prepare(NewRemoteUps, TxId, {remote, node()}),
                            gen_server:reply(Sender, {ok, {specula_commit, NewMaxPrep}}),
...
handle_cast({prepared, PendingTxId, PendingPT}, 
    ...
    case dict:find(PendingTxId, DepDict) of
        {ok, {1, [], OldPrepTime}} -> %% Maybe the transaction can commit 
            %lager:warning("~w has all remote prep, pt is ~w", [PendingTxId, PendingPT]),
            case PendingList of
            ...
            case CurrentTxId of
                ...
                _ ->
                  %lager:warning("Current tx is ~w, Stage is ~w", [CurrentTxId, Stage]),
                    case Stage of
                        local_cert -> 
                            %lager:warning("Has current txn local, Pendinglist is ~w, NumAborted is ~w", [PendingList, Length]),
                            abort_tx(CurrentTxId, LocalParts, [], DoRepl, RepDict),
                            gen_server:reply(Sender, {aborted, CurrentTxId}),
...
add_to_table([], _, _, _, Part, Num) ->
    {Part, Num};
add_to_table([{{Partition, Node}, Updates}|Rest], TxId, PrepareTime, RepDict, Part, Num) ->
    {Ts, NumToAdd} = case dict:find({rep, Node}, RepDict) of
            {ok, DataReplServ} ->
                %lager:warning("Prepareing specula for ~w ~w", [TxId, Partition]),
                {?DATA_REPL_SERV:prepare_specula(DataReplServ, TxId, Partition, Updates, PrepareTime), 1};
            _ ->
                %% Send to local cache.
    ...
    add_to_table(Rest, TxId, PrepareTime, RepDict, [{{Partition, Node}, Updates, max(PrepareTime, Ts)}|Part], Num+NumToAdd).
