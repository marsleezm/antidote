...
start_link(Name) ->
    %lager:waring("Specula tx cert started wit name ~w, id is ~p", [Name, self()]),
...
init([Name]) ->
    ...
    %lager:waring("TotalReplFactor is ~w", [TotalReplFactor]),
...
handle_call({start_tx, SpeculaRead, SpeculaCommit}, _Sender, SD0=#state{dep_dict=D, min_snapshot_ts=MinSnapshotTS, min_commit_ts=MinCommitTS}) ->
    ...
    %lager:waring("Starting txid ~w, MinSnapshotTS is ~w", [TxId, MinSnapshotTS+1]),
...
handle_call({set_int_data, Type, Param}, _Sender, SD0)->
    case Type of
        last_commit_time ->
            %lager:waring("Set lct to ~w", [Param]),
...
handle_call({certify, TxId, LocalUpdates, RemoteUpdates, StartTime},  Sender, SD0=#state{rep_dict=RepDict, cascade_aborted=CascadeAborted, read_invalid=ReadInvalid, total_repl_factor=TotalReplFactor, committed=Committed, pending_txs=PendingTxs, min_commit_ts=LastCommitTs, tx_id=TxId,  invalid_ts=InvalidTs, pending_list=PendingList, dep_dict=DepDict, cdf=Cdf, specula_commit=SpeculaCommit}) ->
    ...
    %lager:waring("Start certifying ~w, readDepTxs is ~w", [TxId, ReadDepTxs]),
    case InvalidTs  of
        0 ->
            case LocalUpdates of
                [] ->
                    ...
                    case RemotePartitions of
                        [] -> %% Is read-only transaction!!
                            {0, ValidDeps, _} = dict:fetch(TxId, DepDict),
                            RemainDeps = ReadDepTxs -- ValidDeps,
                            case RemainDeps of 
                                []  -> gen_server:reply(Sender, {ok, {committed, LastCommitTs}}),
                                            %lager:waring("~w committed with no read dep", [TxId]),
                                ...
                                _ ->  %% Will raise exception if happens
                                      case SpeculaCommit of
                                          ...
                                          false ->
                                            DepDict1 = dict:store(TxId, {read_only, RemainDeps, LastCommitTs}, DepDict),
                                            {noreply, SD0#state{dep_dict=DepDict1, tx_id=TxId, sender=Sender}}
                                      end
                            end;
                        _ ->
                            %%% !!!!!!! Add to ets if is not read only
                            %lager:waring("~w add ~w to ets", [TxId, StartTime]),
                            ...
                            case SpeculaCommit of
                                false -> %% Wait instead of speculate.
                                    ...
                                    DepDict1 = dict:update(TxId, 
                                         fun({_, B, _}) ->   %lager:waring("Previous readdep is ~w", [B]),
                                ...
                                true -> %% Can speculate. After replying, removing TxId
                                      %lager:waring("Speculating directly for ~w", [TxId]),
                                    ...
                                    %lager:waring("Specula txn ~w, got propose time ~w, avoided ~w, still need ~w", [TxId, ProposeTS, AvoidNum, TotalReplFactor*length(RemotePartitions)-AvoidNum]),
                ...
                _ ->
                    ...
                    %lager:waring("Prepare ~w to ~w", [TxId, LocalPartitions]),
                    ?CLOCKSI_VNODE:prepare(LocalUpdates, TxId, local),
                    %lager:waring("Total repl fact is ~w", [TotalReplFactor]),
                    %lager:waring("Total to ack is ~w", [(TotalReplFactor-1)*NumToAck]),
...
handle_cast({pending_prepared, TxId, PrepareTime}, 
    ...
    case dict:find(TxId, DepDict) of
        ...
        {ok, {1, ReadDepTxs, OldPrepTime}} ->
            ...
            case SpeculaCommit of
                false ->
                      %lager:waring("Pedning prep: decided to wait and prepare ~w, pending list is ~w!!", [TxId, PendingList]),
                    ?CLOCKSI_VNODE:prepare(RemoteUpdates, TxId, {remote, ignore}),
                ...
                true ->
                     %lager:waring("Pending prep: decided to speculate ~w and prepare to ~w pending list is ~w!!", [TxId, RemoteUpdates, PendingList]),
                    ...
                    %lager:waring("Specula txn ~w, got propose time ~w, avoided ~w, still need ~w", [TxId, ProposeTS, AvoidNum, TotalReplFactor*RemoteToAck+PendingPrepares+1-AvoidNum]),
                    %lager:waring("Proposets for ~w is ~w", [ProposeTS, TxId]),
                    ?CLOCKSI_VNODE:prepare(RemoteUpdates, ProposeTS, TxId, {remote, node()}),
        ...
        {ok, {N, ReadDeps, OldPrepTime}} ->
              %lager:waring("~w needs ~w local prep replies", [TxId, N-1]),
...
%      %lager:waring("Real prepare in local cert for ~w", [TxId]),
%    {noreply, SD0#state{pending_prepares=PendingPrepares-1}};

%handle_cast({real_prepared, TxId, PrepareTime}, SD0) ->
%       %lager:waring("Real prepare in not local cert ~w", [TxId]),
...
handle_cast({prepared, TxId, PrepareTime}, 
     ...
     %lager:waring("Got local prepare for ~w", [TxId, PrepareTime]),
    case dict:find(TxId, DepDict) of
        %% Maybe can commit already.
        {ok, {1, ReadDepTxs, OldPrepTime}} ->
            NewMaxPrep = max(PrepareTime, OldPrepTime),
                ...
                true ->
                    ...
                    case Cdf of false -> ok;
                                _ -> LastTime = delete_first(Cdf),
                                    %case LastTime of LT -> ok; %lager:waring("First is ~w same as LP", [LastTime]);
                                    %              _ -> %lager:waring("First is ~w, different from LP ~w", [LastTime, LT])
                                    %end,
                    ...
                    case SpeculaCommit of
                        false -> 
                            %%In wait stage, only prepare and doesn't add data to table
                              %lager:waring("Decided to wait and prepare ~w, pending list is ~w!!", [TxId, PendingList]),
                            ?CLOCKSI_VNODE:prepare(RemoteUpdates, TxId, {remote, ignore}),
                            DepDict1 = dict:store(TxId, {TotalReplFactor*RemoteToAck+PendingPrepares, ReadDepTxs, NewMaxPrep}, DepDict),
                            {noreply, SD0#state{dep_dict=DepDict1, stage=remote_cert}};
                        true ->
                              %lager:waring("Speculate current tx with ~w, remote parts are ~w, Num is ~w", [TxId, RemoteParts, length(RemoteParts)]),
                            %% Add dependent data into the table
                            ets:insert(PendingTxs, {TxId, {LocalParts, RemoteParts, LT, os:timestamp(), no_wait}}),
                            {ProposeTS, NumAvoid} = add_to_table(RemoteUpdates, TxId, NewMaxPrep, RepDict),
                            %lager:waring("Specula txn ~w, got propose time ~w, avoided ~w, still need", [TxId, ProposeTS, NumAvoid, TotalReplFactor*RemoteToAck+PendingPrepares-NumAvoid]),
                            %lager:waring("Proposets for ~w is ~w", [ProposeTS, TxId]),
                            ?CLOCKSI_VNODE:prepare(RemoteUpdates, ProposeTS, TxId, {remote, node()}),
                            gen_server:reply(Sender, {ok, {specula_commit, NewMaxPrep}}),
...
handle_cast({prepared, PendingTxId, PendingPT}, 
     ...
     %lager:waring("Got remote prepare for ~w, pt is ~w", [PendingTxId, PendingPT]),
    case dict:find(PendingTxId, DepDict) of
        ...
        {ok, {PrepDeps, ReadDeps, OldPrepTime}} -> %% Maybe the transaction can commit 
            %lager:waring("Not enough.. Prep ~w, Read ~w, PrepTime ~w", [PrepDeps, ReadDeps, OldPrepTime]),
...
handle_cast({read_valid, PendingTxId, PendedTxId}, SD0=#state{dep_dict=DepDict, 
      ...
      %lager:waring("Got read valid for ~w of ~w", [PendingTxId, PendedTxId]),
    case dict:find(PendingTxId, DepDict) of
        {ok, {read_only, [PendedTxId], ReadOnlyTs}} ->
            %lager:waring("Got read valid for ~w, committed", [PendingTxId]),
        ...
        {ok, {read_only, MoreDeps, ReadOnlyTs}} ->
            %lager:waring("Got read valid for ~w, can not commit", [PendingTxId]),
        ...
        {ok, {0, SolvedReadDeps, 0}} -> 
            %% Txn is still reading!!
              %lager:waring("Inserting read valid for ~w, old deps are ~w",[PendingTxId, SolvedReadDeps]),
        ...
        {ok, {PrepDeps, ReadDepTxs, OldPrepTime}} -> 
          %lager:waring("Can not commit... Remaining prepdep is ~w, read dep is ~w", [PrepDeps, lists:delete(PendedTxId, ReadDepTxs)]),
            {noreply, SD0#state{dep_dict=dict:store(PendingTxId, {PrepDeps, lists:delete(PendedTxId, ReadDepTxs), 
                OldPrepTime}, DepDict)}};
        error ->
            %lager:waring("WTF, no info?"),
            {noreply, SD0}
    end;
...
handle_cast({aborted, TxId, FromNode}, SD0=#state{tx_id=TxId, local_updates=LocalParts, 
    ...
    abort_tx(TxId, lists:delete(FromNode, LocalParts), [], RepDict, DepDict),
    DepDict1 = dict:erase(TxId, DepDict),
     %lager:waring("~w local abort! Cert aborted is now ~w", [TxId, CertAborted+1]),
    %lager:waring("~w aborted!", [TxId]),
...
handle_cast({aborted, TxId, FromNode}, 
     ...
     %lager:waring("Aborting ~w remote", [TxId]),
    RemoteParts = [P || {P, _} <- RemoteUpdates],
    abort_tx(TxId, LocalParts, lists:delete(FromNode, RemoteParts), RepDict, DepDict),
    DepDict1 = dict:erase(PendingTxs, DepDict),
     %lager:waring("~w remote abort! Cert aborted is now ~w", [TxId, CertAborted+1]),
    %lager:waring("~w aborted!", [TxId]),
...
handle_cast({aborted, PendingTxId, FromNode}, 
     ...
     %lager:waring("Aborting ~w remote, not current transaction", [PendingTxId]),
    case start_from_list(PendingTxId, PendingList) of
        [] ->
           %lager:waring("Not aborting anything"),
            {noreply, SD0};
        {Prev, L} ->
            Length = length(L),
            ...
            case CurrentTxId of
                ?NO_TXN ->
                     %lager:waring("No current txn, Pendinglist is ~w, NumAborted is ~w", [PendingTxId, Length-1]),
                    {noreply, SD0#state{dep_dict=DepDict1, 
                        pending_list=Prev,
                        cert_aborted=CertAborted+1, cascade_aborted=CascadeAborted+Length-1}};
                _ ->
                   %lager:waring("Trying to abort local txn ~w", [CurrentTxId]),
                    RemoteParts = [P || {P, _} <- RemoteUpdates],
                    case Stage of
                        read ->
                            %% The current transaction is cert_aborted! So replying to client.
                              %lager:waring("~w txn is in reading !!", [CurrentTxId]),
                            {noreply, SD0#state{dep_dict=DepDict1, invalid_ts=?FLOW_ABORT, 
                                pending_list=Prev,
                                cert_aborted=CertAborted+1, cascade_aborted=CascadeAborted+Length-1}};
                        _ ->
                            abort_tx(CurrentTxId, LocalParts, RemoteParts, RepDict, DepDict),
                            DepDict2 = dict:erase(CurrentTxId, DepDict1),
                             %lager:waring("~w remote abort current! Cert aborted is now ~w", [CurrentTxId, CertAborted+1]),
                            %lager:waring("~w aborted!", [CurrentTxId]),
    ...
    case PendingList of
        [] -> %% This is the just report_committed txn.. But new txn has not come yet.
            ...
            case Cdf of false -> ok;
                        _ -> LastTime = delete_first(Cdf),
                            ...
                            %              _ -> %lager:waring("First is ~w, different from LP ~w", [LastTime, LT])
                            %end,
                             %lager:waring("Last time is ~w, Now is ~w", [LastTime, os:timestamp()]),
        ...
        [PendingTxId|Rest] ->
            ...
            case decide_after_cascade(NewPendingList, DepDict2, NumAborted, CurrentTxId, Stage) of
                invalid -> %% Should be cert_aborted later
                    %lager:waring("Current tx invalid"),
                ...
                {abort_local, CFlowAbort, CReadInvalid} ->
                    %lager:waring("Abort local txn"),
                    abort_tx(CurrentTxId, LocalParts, [], RepDict, DepDict),
        ...
        _ ->
            DepDict1 = dict:store(PendingTxId, {0, [], NowPrepTime}, DepDict),
             %lager:waring("got all replies, but I am not the first! PendingList is ~w", [PendingList]),
            {noreply, SD0#state{dep_dict=DepDict1}}
    end.
...
read_abort(Type, MyCommitTime, TxId, SD0=#state{tx_id=CurrentTxId, sender=Sender, read_aborted=ReadAborted,
     ...
     %lager:waring("Got read invalid for ~w", [TxId]),
    {RAD1, RID1} = case Type of read_aborted -> {ReadAborted+1, ReadInvalid};
                                read_invalid -> {ReadAborted, ReadInvalid+1}
                   end,
     %lager:waring("Tx ~w aborted! ReadAborted is ~w, ReadInvalid is ~w", [TxId, RAD1, RID1]),
    case dict:find(TxId, DepDict) of
        {ok, {read_only, _, _}} ->
            %lager:waring("Got read invalid for ~w, abort!", [TxId]),
        ...
        [] ->
            case TxId of
                CurrentTxId ->
                    case Stage of
                        local_cert ->
                            abort_tx(TxId, LocalParts, [], RepDict, DepDict),
                        ...
                        remote_cert ->
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
                            abort_tx(TxId, LocalParts, RemoteParts, RepDict, DepDict),
                            RD1 = dict:erase(TxId, DepDict),
                            %lager:waring("~w aborted!", [TxId]),
                            gen_server:reply(Sender, {aborted, TxId}),
                            {noreply, SD0#state{dep_dict=RD1, tx_id=?NO_TXN, read_aborted=RAD1, read_invalid=RID1}};
        ...
        {Prev, L} ->
            ...
            Length = length(L),
             %lager:waring("Abort due to read invalid, Pending list is ~w, PendingTxId is ~w, List is ~w", [PendingList, TxId, L]),
            case CurrentTxId of
                ?NO_TXN ->
                     %lager:waring("No current txn, Pendinglist is ~w, NumAborted is ~w", [PendingList, Length-1]),
                    {noreply, SD0#state{dep_dict=RD, 
                        pending_list=Prev,
                        read_aborted=RAD1, read_invalid=RID1, cascade_aborted=CascadAborted+Length-1}};
                _ ->
                   %lager:waring("Current tx is ~w, Stage is ~w", [CurrentTxId, Stage]),
                    case Stage of
                        local_cert -> 
                            abort_tx(CurrentTxId, LocalParts, [], RepDict, DepDict),
                            %lager:waring("~w aborted!", [CurrentTxId]),
                        ...
                        remote_cert -> 
                             %lager:waring("Has current txn remote, Pendinglist is ~w, NumAborted is ~w", [PendingList, Length]),
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
                            abort_tx(CurrentTxId, LocalParts, RemoteParts, RepDict, DepDict),
                            %lager:waring("~w aborted!", [CurrentTxId]),
                            gen_server:reply(Sender, {aborted, CurrentTxId}),
                            ...
                            {noreply, SD0#state{dep_dict=RD1, tx_id=?NO_TXN, 
                                ...
                                read_aborted=RAD1, read_invalid=RID1, cascade_aborted=CascadAborted+Length}};
                        read ->
                             %lager:waring("Has current txn read, Pendinglist is ~w, NumAborted is ~w", [PendingList, Length-1]),
                            case (MyCommitTime == -1) or (InvalidTS == -1) of
                                true ->
...
decide_after_cascade(PendingList, DepDict, NumAborted, TxId, Stage) ->
    %lager:waring("PendingList ~w, DepDict ~w, NumAborted ~w, TxId ~w, Stage ~w", [PendingList, DepDict, NumAborted, TxId, Stage]),
    case TxId of
        ?NO_TXN -> wait;
        _ -> 
            case NumAborted > 0 of
                true ->  %lager:waring("Abort due to flow dep"),
                ...
                false ->
                    case dict:find(TxId, DepDict) of %% TODO: should give a invalid_ts 
                        error ->  %lager:waring("Abort due to read dep"),
...
try_to_abort(PendingList, ToAbortTxs, DepDict, RepDict, PendingTxs, ReadAborted) ->
   %lager:waring("Trying to abort ~w ~n", [ToAbortTxs]),
...
commit_tx(TxId, CommitTime, LocalParts, RemoteParts, DepDict, RepDict) ->
    DepList = ets:lookup(dependency, TxId),
     %lager:waring("Committing tx ~w with ~w", [TxId, CommitTime]),
   %lager:waring("~w: My read dependncy are ~w", [TxId, DepList]),
    {DepDict1, ToAbortTxs} = solve_read_dependency(CommitTime, DepDict, DepList),
   %lager:waring("Commit ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
...
commit_specula_tx(TxId, CommitTime, DepDict, RepDict, PendingTxs, Cdf) ->
   %ning("Committing specula tx ~w with ~w", [TxId, CommitTime]),
    ...
    %case First of LP -> ok; %lager:waring("First is ~w same as LP", [First]);
    %              _ -> %lager:waring("First is ~w, different from LP ~w, RP is ~w", [First, LP, RP])
    %end,
    case Cdf of false -> ok;
                _ -> ets:insert(Cdf, {{percv, TxId}, get_time_diff(First, RP)}),
                     %lager:waring("First is ~w, Now is ~w", [First, os:timestamp()]),
    ...
    ?REPL_FSM:repl_commit(RemoteParts, TxId, CommitTime, true, RepDict, IfWaited),
     %lager:waring("Specula commit ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
...
abort_specula_tx(TxId, PendingTxs, RepDict, DepDict, ExceptNode) ->
    [{TxId, {LocalParts, RemoteParts, _LP, _RP, IfWaited}}] = ets:lookup(PendingTxs, TxId),
       %lager:waring("Trying to abort specula ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
      ...
      lists:foreach(fun({_, DepTxId}) ->
                                ...
                                case TxServer of
                                    ...
                                    _ ->
                                         %lager:waring("~w is not my own, read invalid", [DepTxId]),
...
abort_tx(TxId, LocalParts, RemoteParts, RepDict, DepDict) ->
    %lager:waring("Trying to abort ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
    %true = ets:delete(PendingTxs, TxId),
    DepList = ets:lookup(dependency, TxId),
    %lager:waring("~w: My read dependncy are ~w", [TxId, DepList]),
    Self = self(),
...
specula_prepare([{{Partition, Node}, Updates}|Rest], TxId, PrepareTime, RepDict, NumParts) ->
    case dict:find({rep, Node}, RepDict) of
            {ok, DataReplServ} ->
                 %lager:waring("Prepareing specula for ~w ~w", [TxId, Partition]),
...
%   %lager:waring("Committing to table for ~w", [Partition]),
%    case dict:find(Node, RepDict) of
%        error ->
%            %% Send to local cache.
%            ?CACHE_SERV:commit_specula(TxId, Partition, CommitTime);
%        _ -> %{ok, DataReplServ} ->
%            %lager:waring("Sending to ~w", [DataReplServ]),
%            ok
...
%        _ -> %{ok, DataReplServ} ->
%            ok
%              %lager:waring("Aborting specula for ~w ~w", [TxId, Partition]),
...
try_to_commit(LastCommitTime, [], _RepDict, DepDict, _PendingTxs, NumAborted, Committed, _Cdf) ->
     %lager:waring("Returning ~w ~w ~w ~w ~w", [[], LastCommitTime, DepDict, NumAborted, Committed]),
...
try_to_commit(LastCommitTime, [H|Rest]=PendingList, RepDict, DepDict, 
    ...
    Result = case dict:find(H, DepDict) of
     ...
     %lager:waring("If can commit decision for ~w is ~w", [H, Result]),
    case Result of
        false ->
             %lager:waring("Returning ~w ~w ~w ~w ~w", [PendingList, LastCommitTime, DepDict, NumAborted, Committed]),
        ...
        {true, CommitTime} ->
            {DepDict1, ToAbortTxs} = commit_specula_tx(H, CommitTime, 
              ...
              %lager:waring("Before commit specula tx, ToAbortTs are ~w, dict is ~w", [ToAbortTxs, DepDict1]),
            case ToAbortTxs of
                [] ->
...
abort_specula_list([H|T], RepDict, DepDict, PendingTxs, ExceptNode) ->
     %lager:waring("Trying to abort ~w", [H]),
...
solve_read_dependency(CommitTime, ReadDep, DepList) ->
    ...
    lists:foldl(fun({TxId, DepTxId}, {RD, ToAbort}) ->
                    ...
                    case DepTxId#tx_id.snapshot_time >= CommitTime of
                        ...
                        true ->
                          %lager:waring("Read still valid for ~w, CommitTime is ~w", [DepTxId, CommitTime]),
                            case TxServer of
                                Self ->
                                  %lager:waring("~w is my own, read valid", [DepTxId]),
                                    case dict:find(DepTxId, RD) of
                                        ...
                                        {ok, {read_only, _SolvedReadDeps, _}} -> %% Local transaction is still reading
                                           %lager:waring("~w reducing read dep ~w", [TxId]),
                                            ?READ_VALID(Self, DepTxId, TxId),
                                            {RD, ToAbort};
                                        {ok, {PrepDeps, ReadDeps, PrepTime}} ->
                                          %lager:waring("Storing ~w for ~w", [lists:delete(TxId, ReadDeps), DepTxId]),
                                        ...
                                        error -> %% This txn hasn't even started certifying 
                                           ...
                                           %lager:waring("This txn has not even started"),
                                            {RD, ToAbort}
                                    end;
                                _ ->
                                   %lager:waring("~w is not my own, read valid", [DepTxId]),
                                ...
                                Self ->
                                    case dict:find(DepTxId, RD) of
                                        {ok, {read_only, _SolvedReadDeps, _}} -> 
                                               %lager:waring("~w read invalid!", [TxId]),
...
abort_tx_test() ->
    ...
    LocalParts = [lp1, lp2],
    TxId1 = tx_utilities:create_tx_id(0),
    abort_tx(TxId1, LocalParts, [], dict:new(), dict:new()),
