...
start_link(Name) ->
  %lager:warrning("Specula tx cert started wit name ~w", [Name]),
...
handle_call({start_tx}, _Sender, SD0=#state{dep_dict=D, min_snapshot_ts=MinSnapshotTS, min_commit_ts=MinCommitTS}) ->
    ...
    %lager:warrning("Starting txid ~w, MinSnapshotTS is ~w", [TxId, MinSnapshotTS+1]),
...
handle_call({get_stat}, _Sender, SD0=#state{cert_aborted=CertAborted, committed=Committed, read_aborted=ReadAborted, cascade_aborted=CascadeAborted, num_specula_read=NumSpeculaRead, pending_txs=PendingTxs, cert_stat={AccT, AccN}}) ->
  %lager:warrning("Num of read cert_aborted ~w, Num of cert_aborted is ~w, Num of committed is ~w, NumSpeculaRead is ~w", [ReadAborted, CascadeAborted, Committed, NumSpeculaRead]),
...
handle_call({set_int_data, Type, Param}, _Sender, SD0)->
    case Type of
        last_commit_time ->
          %lager:warrning("Set lct to ~w", [Param]),
...
handle_call({certify, TxId, LocalUpdates, RemoteUpdates},  Sender, SD0=#state{rep_dict=RepDict, read_aborted=ReadAborted, 
    ...
    case InvalidTs  of
                        ...
                        _ ->
                            ...
                            case length(PendingList) >= SpeculaLength of
                                ...
                                false -> %% Can speculate. After replying, removing TxId
                                    %lager:warrning("Speculating directly for ~w", [TxId]),
        ...
        -1 -> 
          ...
          %lager:warrning("Aborting txn due to invalidTS ~w!", [TxId]),
...
handle_cast({pending_prepared, TxId, PrepareTime}, 
    ...
    %lager:warrning("Speculative receive pending_prepared for ~w, current pp is ~w", [TxId, PendingPrepares+1]),
    case dict:find(TxId, DepDict) of
        ...
        {ok, {1, ReadDepTxs, OldPrepTime}} ->
            ...
            case length(PendingList) >= SpeculaLength of
                true ->
                    %lager:warrning("Pedning prep: decided to wait and prepare ~w, pending list is ~w!!", [TxId, PendingList]),
                    ?CLOCKSI_VNODE:prepare(RemoteUpdates, TxId, {remote, ignore}),
                ...
                false ->
                   %lager:warrning("Pending prep: decided to speculate ~w, pending list is ~w!!", [TxId, PendingList]),
        ...
        {ok, {N, ReadDeps, OldPrepTime}} ->
            %lager:warrning("~w needs ~w local prep replies", [TxId, N-1]),
...
handle_cast({real_prepared, TxId, _}, SD0=#state{pending_prepares=PendingPrepares, tx_id=TxId, stage=local_cert}) ->
    %lager:warrning("Real prepare in local cert for ~w", [TxId]),
    {noreply, SD0#state{pending_prepares=PendingPrepares-1}};

handle_cast({real_prepared, TxId, PrepareTime}, SD0) ->
     %lager:warrning("Real prepare in not local cert ~w", [TxId]),
...
handle_cast({prepared, TxId, PrepareTime}, 
    ...
    %lager:warrning("Got local prepare for ~w", [TxId]),
    case dict:find(TxId, DepDict) of
        %% Maybe can commit already.
        {ok, {1, ReadDepTxs, OldPrepTime}} ->
            NewMaxPrep = max(PrepareTime, OldPrepTime),
                ...
                false ->
                    case length(PendingList) >= SpeculaLength of
                        true -> 
                            ...
                            %lager:warrning("Decided to wait and prepare ~w, pending list is ~w!!", [TxId, PendingList]),
                            ?CLOCKSI_VNODE:prepare(RemoteUpdates, TxId, {remote, ignore}),
                            DepDict1 = dict:store(TxId, {RemoteToAck+PendingPrepares, ReadDepTxs, NewMaxPrep}, DepDict),
                            {AccTime, AccCount} = CertStat,
                            {noreply, SD0#state{dep_dict=DepDict1, stage=remote_cert, cert_stat={AccTime+timer:now_diff(os:timestamp(),StartPrepare), AccCount+1}}};
                        false ->
                            %lager:warrning("Speculate current tx with ~w, remote parts are ~w, Num is ~w", [TxId, RemoteParts, length(RemoteParts)]),
                            %% Add dependent data into the table
                            ets:insert(PendingTxs, {TxId, {LocalParts, RemoteParts, os:timestamp()}}),
                            NewRemoteUps = add_to_table(RemoteUpdates, TxId, NewMaxPrep, RepDict),
                            ?CLOCKSI_VNODE:prepare(NewRemoteUps, TxId, {remote, node()}),
...
handle_cast({prepared, PendingTxId, PendingPT}, 
	    SD0=#state{pending_list=PendingList, do_repl=DoRepl, pending_txs=PendingTxs,
            ...
            committed=Committed, read_aborted=ReadAborted, cascade_aborted=CascadAborted, remote_updates=RemoteUpdates,
            local_updates=LocalParts, dep_dict=DepDict, stage=Stage}) ->
    %lager:warrning("Got remote prepare for ~w", [PendingTxId]),
    case dict:find(PendingTxId, DepDict) of
        {ok, {1, [], OldPrepTime}} -> %% Maybe the transaction can commit 
            %lager:warrning("~w has all remote prep", [PendingTxId]),
            case PendingList of
                [] -> %% This is the just report_committed txn.. But new txn has not come yet.
                    %lager:warrning("Pending list is ~w", [PendingList]),
                ...
                [PendingTxId|_] ->
                    ...
                    %lager:warrning("got all replies, Can try to commit"),
                    ...
                    case decide_after_cascade(NewPendingList, DepDict2, NumAborted, CurrentTxId, Stage) of
                        ...
                        abort_remote ->
                          %lager:warrning("Abort remote txn"),
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
                            ...
                            {noreply, SD0#state{
                              pending_list=NewPendingList, tx_id=?NO_TXN, dep_dict=DepDict3,
                                min_commit_ts=NewMaxPT,  read_aborted=ReadAborted+min(NumAborted, 1),
                                      cascade_aborted=CascadAborted+NumAborted, committed=NewCommitted}};
                        wait -> 
                          %lager:warrning("Local txn waits"),
                            {noreply, SD0#state{pending_list=NewPendingList, dep_dict=DepDict2, 
                                    read_aborted=ReadAborted+min(NumAborted, 1),
                                      cascade_aborted=CascadAborted+NumAborted, committed=NewCommitted}};
                        {commit, OldCurPrepTime} ->
                          %lager:warrning("Commit local txn"),
                            ...
                            gen_server:reply(Sender, {ok, {committed, CurCommitTime}}),
                            {noreply, SD0#state{min_commit_ts=CurCommitTime, tx_id=?NO_TXN, read_aborted=ReadAborted+min(NumAborted, 1),
                        ...
                        specula -> %% Can not commit, but can specualte (due to the number of specula txns decreased)
                          %lager:warrning("Specula current txn ~w", [CurrentTxId]),
                ...
                _ ->
                    DepDict1 = dict:store(PendingTxId, {0, [], max(PendingPT, OldPrepTime)}, DepDict),
                    %lager:warrning("got all replies, but I am not the first! PendingList is ~w", [PendingList]),
                    {noreply, SD0#state{dep_dict=DepDict1}}
            end;
        {ok, {PrepDeps, ReadDeps, OldPrepTime}} -> %% Maybe the transaction can commit 
           %lager:warrning("Not enough.. Prep ~w, Read ~w, PrepTime ~w", [PrepDeps, ReadDeps, OldPrepTime]),
...
handle_cast({read_valid, PendingTxId, PendedTxId}, SD0=#state{pending_txs=PendingTxs, rep_dict=RepDict, dep_dict=DepDict, 
    ...
    %lager:warrning("Got read valid for ~w of ~w", [PendingTxId, PendedTxId]),
    case dict:find(PendingTxId, DepDict) of
        ...
        {ok, {0, SolvedReadDeps, 0}} -> 
            ...
            %lager:warrning("Inserting read valid for ~w, old deps are ~w",[PendingTxId, SolvedReadDeps]),
        ...
        {ok, {0, [PendedTxId], OldPrepTime}} -> %% Maybe the transaction can commit 
            case PendingList of
                 [] ->
                    %lager:warrning("Committing current txn... Current txn id ~w should be the same as  pending id~w", [CurrentTxId, PendingTxId]),
                ...
                _ ->
                    DepDict1 = dict:store(PendingTxId, {0, [], OldPrepTime}, DepDict),
                  %lager:warrning("got all replies, but I am not the first!"),
                    {noreply, SD0#state{dep_dict=DepDict1}}
            end;
        {ok, {PrepDeps, ReadDepTxs, OldPrepTime}} -> 
         %lager:warrning("Can not commit... Remaining prepdep is ~w, read dep is ~w", [PrepDeps, lists:delete(PendedTxId, ReadDepTxs)]),
            {noreply, SD0#state{dep_dict=dict:store(PendingTxId, {PrepDeps, lists:delete(PendedTxId, ReadDepTxs), 
                OldPrepTime}, DepDict)}};
        error ->
          %lager:warrning("WTF, no info?"),
            {noreply, SD0}
    end;

handle_cast({read_invalid, MyCommitTime, TxId}, SD0=#state{tx_id=CurrentTxId, sender=Sender, read_aborted=ReadAborted, 
   ...
   %lager:warrning("Got read invalid for ~w", [TxId]),
    case dict:find(TxId, DepDict) of
        {ok, {read_only, _, _}} ->
                ...
                CurrentTxId ->
                    case Stage of
                        local_cert ->
                            ...
                            {noreply, SD0#state{dep_dict=RD1, tx_id=?NO_TXN, read_aborted=ReadAborted+1}};
                        remote_cert ->
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
        ...
        L ->
           ...
           %lager:warrning("Abort due to read invalid, Pending list is ~w, PendingTxId is ~w, List is ~w", [PendingList, TxId, L]),
            case CurrentTxId of
                ?NO_TXN ->
                    {noreply, SD0#state{dep_dict=RD, 
                        pending_list=lists:sublist(PendingList, length(PendingList)-length(L)),
                        read_aborted=ReadAborted+1, cascade_aborted=CascadAborted+length(L)-1}};
                _ ->
                 %lager:warrning("Current tx is ~w, Stage is ~w", [CurrentTxId, Stage]),
                    Length = length(L),
                    case Stage of
                        local_cert -> 
                            abort_tx(CurrentTxId, LocalParts, [], DoRepl, RepDict),
                            ...
                            {noreply, SD0#state{dep_dict=RD1, tx_id=?NO_TXN, 
                                ...
                                read_aborted=ReadAborted+1, cascade_aborted=CascadAborted+Length}};
                        remote_cert -> 
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
                            abort_tx(CurrentTxId, LocalParts, RemoteParts, DoRepl, RepDict),
                            gen_server:reply(Sender, {aborted, CurrentTxId}),
                            ...
                            case (MyCommitTime == -1) or (InvalidTS == -1) of
                                true ->
                                    {noreply, SD0#state{dep_dict=RD, invalid_ts=-1,
                                        pending_list=lists:sublist(PendingList, length(PendingList)-Length),
                                        read_aborted=ReadAborted+1, cascade_aborted=CascadAborted+Length-1}};
                                _ -> 
...
handle_cast({aborted, TxId, FromNode}, 
   ...
   %lager:warrning("Aborting ~w remote", [TxId]),
    RemoteParts = [P || {P, _} <- RemoteUpdates],
...
handle_cast({aborted, PendingTxId, FromNode}, 
   ...
   %lager:warrning("Aborting ~w remote, not current transaction", [PendingTxId]),
    case start_from_list(PendingTxId, PendingList) of
        [] ->
         %lager:warrning("Not aborting anything"),
            {noreply, SD0};
        L ->
           %lager:warrning("Abort remote again, Pending list is ~w, PendingTxId is ~w, List is ~w", [PendingList, PendingTxId, L]),
            Length = length(L),
            ...
            case CurrentTxId of
                ...
                _ ->
                 %lager:warrning("Trying to abort local txn ~w", [CurrentTxId]),
                    RemoteParts = [P || {P, _} <- RemoteUpdates],
                    case Stage of
                        read ->
                            %% The current transaction is cert_aborted! So replying to client.
                            %lager:warrning("~w txn is in reading !!", [CurrentTxId]),
...
decide_after_cascade(PendingList, DepDict, NumAborted, TxId, Stage) ->
 %lager:warrning("PendingList ~w, DepDict ~w, NumAborted ~w, TxId ~w, Stage ~w", [PendingList, DepDict, NumAborted, TxId, Stage]),
    case TxId of
        ?NO_TXN -> wait;
        _ -> 
...
try_to_abort(PendingList, ToAbortTxs, DepDict, RepDict, PendingTxs, DoRepl, ReadAborted) ->
 %lager:warrning("Trying to abort ~w ~n", [ToAbortTxs]),
    MinIndex = find_min_index(ToAbortTxs, PendingList),
    FullLength = length(PendingList),
 %lager:warrning("Min index is ~w, FulLength is ~w, ReadAborted is ~w ~n", [MinIndex, FullLength, ReadAborted]),
...
commit_tx(TxId, CommitTime, LocalParts, RemoteParts, DoRepl, DepDict, RepDict) ->
 %lager:warrning("Committing tx ~w with ~w", [TxId, CommitTime]),
    DepList = ets:lookup(dependency, TxId),
  %lager:warrning("~w: My read dependncy are ~w", [TxId, DepList]),
    {DepDict1, ToAbortTxs} = solve_read_dependency(CommitTime, DepDict, DepList),
   %lager:warrning("Commit ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
...
commit_specula_tx(TxId, CommitTime, DoRepl, DepDict, RepDict, PendingTxs) ->
  %lager:warrning("Committing specula tx ~w with ~w", [TxId, CommitTime]),
  ...
  %lager:warrning("~w: My read dependncy are ~w", [TxId, DepList]),
   ...
   %lager:warrning("Specula commit ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
...
abort_specula_tx(TxId, DoRepl, PendingTxs, RepDict, DepDict, ExceptNode) ->
          [{TxId, {LocalParts, RemoteParts, SpeculaTime}}] = ets:lookup(PendingTxs, TxId),
      %lager:warrning("Trying to abort specula ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
     %lager:warrning("Trying to abort specula ~w", [TxId]),
      ...
      lists:foreach(fun({_, DepTxId}) ->
                                ...
                                case TxServer of
                                    Self ->
                                        ok;
                                    _ ->
                                     %lager:warrning("~w is not my own, read invalid", [DepTxId]),
...
abort_tx(TxId, LocalParts, RemoteParts, DoRepl, RepDict) ->
   %lager:warrning("Trying to abort ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
    %true = ets:delete(PendingTxs, TxId),
    %lager:warrning("Trying to abort specula ~w", [TxId]),
    DepList = ets:lookup(dependency, TxId),
   %lager:warrning("~w: My read dependncy are ~w", [TxId, DepList]),
...
add_to_table([{{Partition, Node}, Updates}|Rest], TxId, PrepareTime, RepDict) ->
    Ts = case dict:find({rep, Node}, RepDict) of
            {ok, DataReplServ} ->
               %lager:warrning("Prepareing specula for ~w ~w", [TxId, Partition]),
...
% %lager:warrning("Committing to table for ~w", [Partition]),
%    case dict:find(Node, RepDict) of
%        error ->
%            %% Send to local cache.
%            ?CACHE_SERV:commit_specula(TxId, Partition, CommitTime);
%        _ -> %{ok, DataReplServ} ->
%          %lager:warrning("Sending to ~w", [DataReplServ]),
%            ok
...
%        _ -> %{ok, DataReplServ} ->
%            ok
%            %lager:warrning("Aborting specula for ~w ~w", [TxId, Partition]),
...
try_to_commit(LastCommitTime, [], _RepDict, DepDict, _DoRepl, _PendingTxs, NumAborted, Committed) ->
     %lager:warrning("Returning ~w ~w ~w ~w ~w", [[], LastCommitTime, DepDict, NumAborted, Committed]),
...
try_to_commit(LastCommitTime, [H|Rest]=PendingList, RepDict, DepDict, 
   ...
   %lager:warrning("Checking if can commit tx ~w", [H]),
    ...
    %lager:warrning("If can commit decision for ~w is ~w", [H, Result]),
    case Result of
        false ->
            %lager:warrning("Returning ~w ~w ~w ~w ~w", [PendingList, LastCommitTime, DepDict, NumAborted, Committed]),
        ...
        {true, CommitTime} ->
            ...
            %lager:warrning("Before commit specula tx, ToAbortTs are ~w, dict is ~w", [ToAbortTxs, DepDict1]),
            case ToAbortTxs of
                [] ->
...
abort_specula_list([H|T], RepDict, DepDict, DoRepl, PendingTxs, ExceptNode) ->
   %lager:warrning("Trying to abort ~w", [H]),
...
solve_read_dependency(CommitTime, ReadDep, DepList) ->
    ...
    lists:foldl(fun({TxId, DepTxId}, {RD, ToAbort}) ->
                    ...
                    case DepTxId#tx_id.snapshot_time >= CommitTime of
                        ...
                        true ->
                        %lager:warrning("Read still valid for ~w", [DepTxId]),
                            case TxServer of
                                Self ->
                                %lager:warrning("~w is my own, read valid", [DepTxId]),
                                    case dict:find(DepTxId, RD) of
                                        {ok, {0, _SolvedReadDeps, 0}} -> %% Local transaction is still reading
                                        %lager:warrning("Deleting {~w, ~w} from antidep", [DepTxId, TxId]),
                                            ets:delete_object(anti_dep, {DepTxId, TxId}), 
                                            {RD, ToAbort};
                                        {ok, {PrepDeps, ReadDeps, PrepTime}} ->
                                        %lager:warrning("Storing ~w for ~w", [lists:delete(TxId, ReadDeps), DepTxId]),
                                        ...
                                        error -> %% This txn hasn't even started certifying 
                                         ...
                                         %lager:warrning("This txn has not even started"),
                                            {RD, ToAbort}
                                    end;
                                _ ->
                                 %lager:warrning("~w is not my own, read valid", [DepTxId]),
                        ...
                        false ->
                            ...
                            case TxServer of
                                Self ->
                                %lager:warrning("~w is my own, read invalid", [DepTxId]),
                                    {RD, [DepTxId|ToAbort]};
                                _ ->
                                %lager:warrning("~w is not my own, read invalid", [DepTxId]),
