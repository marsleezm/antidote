...
init([]) ->
    ...
    %% RepDict stores the name of replicated servers in form of: {rep, node}, name_of_server
    %% Its also stores the replica list of non-local nodes in form of: node, [{rep, name}|name] or node, [cache|name]
    RepDict = hash_fun:build_rep_dict(DoRepl),
    %lager:info("RepDict is ~w", [dict:to_list(RepDict)]),
...
handle_call({certify, TxId, _, _},  _Sender, SD0=#state{tx_id=?NO_TXN, dep_dict=DepDict}) ->
   %lager:warning("Current tx is aborted, received certify of ~w", [TxId]),
                    ...
                    case decide_after_cascade(NewPendingList, DepDict2, NewReadAborted-ReadAborted, CurrentTxId, Stage) of
                        invalid -> %% Should be aborted later
                         %lager:warning("Tnx invalid"),
                            {noreply, SD0#state{pending_list=NewPendingList, dep_dict=DepDict2, invalid_ts=?FLOW_ABORT,
                                  min_commit_ts=NewMaxPT, read_aborted=NewReadAborted, 
                                    committed=NewCommitted}};
                        abort_local ->
                         %lager:warning("Abort local Txn"),
                            abort_tx(CurrentTxId, LocalParts, [], DoRepl, PendingTxs, RepDict),
                        ...
                        abort_remote ->
                         %lager:warning("Abort remote Txn"),
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
                            abort_tx(CurrentTxId, LocalParts, RemoteParts, DoRepl, PendingTxs, RepDict),
                            DepDict3 = dict:erase(CurrentTxId, DepDict2),
                            gen_server:reply(Sender, {aborted, CurrentTxId}),
                            {noreply, SD0#state{
                              pending_list=NewPendingList, tx_id=?NO_TXN, dep_dict=DepDict3, 
        ...
        [] ->
            case TxId of
                CurrentTxId ->
                    case Stage of
                        local_cert ->
                            abort_tx(TxId, LocalParts, [], DoRepl, PendingTxs, RepDict),
                            RD1 = dict:erase(TxId, DepDict),
                            gen_server:reply(Sender, {aborted, TxId}),
                            {noreply, SD0#state{dep_dict=RD1, tx_id=?NO_TXN}};
                        remote_cert ->
                            RemoteParts = [P||{P, _} <-RemoteUpdates],
                            abort_tx(TxId, LocalParts, RemoteParts, DoRepl, PendingTxs, RepDict),
                            RD1 = dict:erase(TxId, DepDict),
                            gen_server:reply(Sender, {aborted, TxId}),
                            {noreply, SD0#state{dep_dict=RD1, tx_id=?NO_TXN}};
                        read ->
...
handle_cast({abort, ReceivedTxId, local, FromNode}, SD0=#state{tx_id=TxId, local_updates=LocalParts, 
        do_repl=DoRepl, sender=Sender, pending_txs=PendingTxs, dep_dict=DepDict, rep_dict=RepDict}) ->
    case ReceivedTxId of
        TxId ->
         %lager:warning("Received local abort for ~w", [TxId]),
            abort_tx(TxId, lists:delete(FromNode, LocalParts), [], DoRepl, PendingTxs, RepDict),
...
handle_cast({abort, TxId, {remote,_}, FromNode}, 
	    SD0=#state{tx_id=TxId, pending_txs=PendingTxs,  do_repl=DoRepl, local_updates=LocalParts, 
            remote_updates=RemoteUpdates, dep_dict=DepDict, sender=Sender, rep_dict=RepDict}) ->
 %lager:warning("Aborting ~w remote", [TxId]),
    RemoteParts = [P || {P, _} <- RemoteUpdates],
    abort_tx(TxId, LocalParts, lists:delete(FromNode, RemoteParts), DoRepl, PendingTxs, RepDict),
...
abort_specula_tx(TxId, LocalParts, RemoteParts, DoRepl, PendingTxs, RepDict, DepDict, ExceptNode) ->
 ...
 %lager:warning("Trying to abort specula ~w to ~w, ~w", [TxId, LocalParts, RemoteParts]),
    abort_tx(TxId, LocalParts, lists:delete(ExceptNode, RemoteParts), DoRepl, PendingTxs, RepDict),
...
abort_specula_tx(TxId, LocalParts, RemoteParts, DoRepl, PendingTxs, RepDict, DepDict) ->
    ...
    lists:foreach(fun({_, DepTxId}) ->
                    ...
                    case TxServer of
                        Self -> ok;
                        _ -> ?READ_INVALID(TxServer, -1, DepTxId)
                  end end, DepList),
...
add_to_table([{{Partition, Node}, Updates}|Rest], TxId, PrepareTime, RepDict) ->
   %lager:warning("Adding ~w, ~w for ~w", [Partition, Node, TxId]),
    case dict:find({rep, Node}, RepDict) of
        {ok, DataReplServ} ->
           %lager:warning("Data repl Adding ~w for ~w", [TxId, DataReplServ]),
            ?DATA_REPL_SERV:prepare_specula(DataReplServ, TxId, Partition, Updates, PrepareTime);
        _ ->
            %% Send to local cache.
           %lager:warning("Cache adding ~w", [TxId]),
...
abort_tx_test() ->
    ...
    abort_tx(TxId1, LocalParts, [], true, MyTable, ignore),
...
commit_tx_test() ->
    ...
    ets:insert(MyTable, {TxId1, whatever}),
    CT = 10000,
    commit_tx(TxId1, CT, LocalParts, [], true, dict:new(), ignore),
