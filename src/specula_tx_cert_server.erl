                        ...
                        _ ->
                            ...
                            case length(PendingList) >= SpeculaLength of
                                ...
                                false -> %% Can speculate. After replying, removing TxId
                                    ...
                                    ProposeTS = add_to_table(RemoteUpdates, TxId, LastCommitTs, RepDict),
        ...
        {ok, {1, ReadDepTxs, OldPrepTime}} ->
            ...
            case length(PendingList) >= SpeculaLength of
                ...
                false ->
                    ...
                    ProposeTS = add_to_table(RemoteUpdates, TxId, NewMaxPrep, RepDict),
                    ?CLOCKSI_VNODE:prepare(RemoteUpdates, ProposeTS, TxId, {remote, node()}),
...
add_to_table(WriteSet, TxId, PrepareTime, RepDict) ->
    ...
    specula_prepare(WriteSet, TxId, ProposeTS, RepDict).

specula_prepare([], _, PrepareTime, _) ->
    PrepareTime;
specula_prepare([{{Partition, Node}, Updates}|Rest], TxId, PrepareTime, RepDict) ->
    case dict:find({rep, Node}, RepDict) of
            {ok, DataReplServ} ->
    ...
    specula_prepare(Rest, TxId, PrepareTime, RepDict).
